\begin{figure}[H]
    ...
    \end{tikzpicture}
    \caption[Segmentation-Classification Pipeline]{A proposed pipeline in which optional model heads can be used on SAM~3 outputs for specific use cases. For high recall in which per species accuracy is non essential, users would use SAM~3 alone. For cases in which a greater volume of correct detections is essential, users would use the three-way model head. For a balanced model, users could use the two-way downstream classifier.}
    \label{fig:pipeline}
\end{figure}