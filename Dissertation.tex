...
Alongside SAM~3, the SA-FARI dataset was released, as well as the fine-tuned model weights derived from the dataset, aiming to improve on the default model for our purpose~\cite{wasmuht2025safaridatasetsegmentfootage}.
...
African Program, is a set of 20,000 static camera trap videos (roughly 7 million frames) featuring chimpanzees and gorillas in their natural habitats, \cite{brookes2024panaf20k, brookes2024panaf20kdataset} taken across 18 field sites in tropical Africa.

PanAf20K contains a fully annotated subset of 500 videos (over 180K frames), referred to as PanAf500. These fine-grained annotations feature species classification, behavioural actions, bounding boxes and associated intra-video tracking IDs. The remainder of PanAf20K contains multi-label annotations indicating behaviours present in the video without associated apes or spacial locations.
...
MegaDetector~\cite{beery2019efficientpipelinecameratrap} is a commonly used tool for detection of animals in camera trap imagery, capable of filtering out empty frames and providing bounding boxes describing the animals' locations.  While this is a great tool for automated detection, it comes with its own disadvantages. For one, the model is not able to classify animals, calling for further manual or automated methods when classification is necessary. Second, bounding boxes do not provide the exact spatial locations, but give a boundary in which the object is contained. Thus, despite excellent detection performance, users of MegaDetector must conduct further review~\cite{velez2022choosingappropriateplatformworkflow}.

\subsection{Classification}

Norouzzadeh et al.~\cite{norouzzadeh2017automaticallyidentifyingcountingdescribing} trained a range of models, including ResNet, on 3.2 million images featuring 48 species. ResNet achieved a top-1 accuracy of 93.8\% in identifying species in images, establishing the viability of CNNs as species classifiers. Further research shows that fine-tuning existing models, even if the base does not coincide with the target dataset, often outperforms CNNs built from scratch~\cite{willi2019identifying}.


\subsection{Great Ape Detection and Recognition}

A large amount of work for analysing camera trap imagery featuring great apes specifically has been and will continue to be led by the University of Bristol.

Yang et al.~\cite{yang2019greatapedetectionchallenging} designed a great ape detector, utilising temporal information and full-body morphology to improve detections in challenging environments when compared with frame based detectors.

Schofield et al.~\cite{schofield2019chimpanzee} proposed a fully automated pipeline using CNNs to detect, track and recognise chimpanzees by their faces. Brust et al.~\cite{brust2017gorilla} conducted similar research combining detection and classification models for gorillas.

These works establish the method of detecting then classifying great apes, which is adapted in this work.
...
SAM~3~\cite{carion2025sam3segmentconcepts} introduces text-based prompting, newly referred to as Promptable Concept Segmentation (PCS) by the author. Given a prompt (e.g. ``chimpanzee") the model will localise and track objects between frames. This functions similarly to SAM~2, but instead of the user inputting where the object is, they indicate what they are looking for.
...
\end{equation}
where $IDSW$ denotes the number of identity switches and $GT$ denotes the number of ground truth objects. Note, that even if $IDSW = 0$, this MOTA can be negative if $FN + FP > GT$, so a low MOTA does not necessarily indicate that tracking alone is underperforming.
